...
export async function GET() {
  try {
    ...
    const folderId = '1bNZD-938qEvYVOY6fYLajuvecbsxET9X'; // Sparring folder ID
    
    console.log('Fetching sparring photos from folder:', folderId);
    ...
    console.log('Found', response.data.files?.length, 'sparring photos');
    ...
    return NextResponse.json({ error: 'Failed to fetch sparring photos' }, { status: 500 });
  }
}
