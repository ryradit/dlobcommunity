      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-3 bg-gradient-to-br from-[#3e6461] to-[#2d4a47] rounded-xl">
              <Dumbbell className="w-8 h-8" />
            </div>
            <div>
              <h1 className="text-3xl font-bold">Training Center</h1>
              <p className="text-zinc-400 text-sm">Tingkatkan skill badminton Anda dengan panduan AI</p>
            </div>
          </div>
        ...
        {/* Search Bar */}
        <div className="training-search-bar bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6 mb-6">
          <form onSubmit={handleSubmit} className="flex gap-3">
            <input
              ...
              disabled={isLoading}
              className="flex-1 px-5 py-4 bg-zinc-800/60 border border-white/10 rounded-xl text-white placeholder-zinc-500 focus:outline-none focus:border-[#3e6461]/50 focus:ring-2 focus:ring-[#3e6461]/20 disabled:opacity-50 transition-all"
            />
            <button
              type="submit"
              disabled={!query.trim() || isLoading}
              className="px-6 py-4 bg-gradient-to-br from-[#3e6461] to-[#2d4a47] hover:from-[#3e6461]/90 hover:to-[#2d4a47]/90 disabled:from-zinc-700 disabled:to-zinc-700 disabled:cursor-not-allowed rounded-xl font-semibold transition-all flex items-center gap-2"
            >
              {isLoading ? (
                <>
          ...
          <div className="training-popular-topics mt-6">
            <p className="text-xs text-zinc-400 font-semibold mb-3">Topik Populer:</p>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
              {popularTopics.map((topic) => (
                <button
                  ...
                  disabled={isLoading}
                  className="flex items-center gap-2 px-3 py-2.5 bg-zinc-800/40 hover:bg-zinc-800/60 border border-white/5 hover:border-white/10 rounded-lg transition-all text-left disabled:opacity-50"
                >
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="training-main-content lg:col-span-2 space-y-6">
            {/* AI Advice */}
            {currentAdvice && (
              <div className="training-ai-advice bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-4">
                  <div className="p-2 bg-[#3e6461]/20 rounded-lg">
                    <Dumbbell className="w-5 h-5 text-[#3e6461]" />
                  </div>
                  <h2 className="text-xl font-bold">Saran Pelatih AI</h2>
                </div>
                <p className="text-zinc-300 leading-relaxed">{currentAdvice}</p>
              </div>
            )}

            {/* Video Recommendations */}
            {currentVideos.length > 0 && (
              <div className="training-video-recommendations bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-4">
                ...
                <div className="space-y-3">
                  {currentVideos.map((video) => (
                    ...
                    >
                      <div className="flex gap-4 bg-zinc-800/40 hover:bg-zinc-800/60 p-4 rounded-xl border border-white/5 hover:border-white/10 transition-all cursor-pointer">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-zinc-200 line-clamp-2 mb-2 group-hover:text-white transition-colors">
                            {video.title}
                          </h3>
                          <p className="text-sm text-zinc-500 mb-3">{video.channelTitle}</p>
            ...
            {!currentAdvice && !isLoading && (
              <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-12 text-center">
                <div className="inline-flex p-4 bg-[#3e6461]/20 rounded-full mb-4">
                  <Dumbbell className="w-12 h-12 text-[#3e6461]" />
                </div>
                <h3 className="text-xl font-bold mb-2">Mulai Latihan Anda</h3>
                ...
                <div className="flex justify-center gap-3">
                  <button
                    ...
                    className="px-4 py-2 bg-zinc-800/60 hover:bg-zinc-800/80 rounded-lg text-sm transition-all"
                  >
                    Contoh Pertanyaan
                  </button>
                </div>
          ...
          <div className="space-y-6">
            <div className="training-history bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
              <h2 className="text-lg font-bold mb-4">Riwayat Latihan</h2>
              {loadingHistory ? (
                ...
                <div className="space-y-2">
                  {history.map((session) => (
                    <div
                      key={session.id}
                      className="group flex items-start gap-2 p-3 bg-zinc-800/40 hover:bg-zinc-800/60 rounded-lg border border-white/5 hover:border-white/10 transition-all"
                    >
                      <button
                        onClick={() => loadHistorySession(session)}
                        className="flex-1 text-left"
                      >
                        <p className="text-sm font-medium text-zinc-300 line-clamp-2 mb-1">
                          {session.query}
                        </p>
            ...
            {/* Tips */}
            <div className="training-tips bg-gradient-to-br from-[#3e6461]/20 to-[#2d4a47]/20 backdrop-blur-xl border border-[#3e6461]/30 rounded-2xl p-6">
              ...
              <ul className="space-y-2 text-sm text-zinc-300">
                <li className="flex gap-2">
                  <span className="text-[#3e6461]">•</span>
                  <span>Jelaskan masalah spesifik Anda</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-[#3e6461]">•</span>
                  <span>Sebutkan level skill Anda</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-[#3e6461]">•</span>
                  <span>Fokus pada satu topik per pertanyaan</span>
                </li>
                <li className="flex gap-2">
                  <span className="text-[#3e6461]">•</span>
                  <span>Tonton video tutorial untuk hasil terbaik</span>
                </li>
              </ul>
            </div>
          </div>
