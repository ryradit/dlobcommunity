      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-white">Keuangan</h1>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="keuangan-add-button px-4 py-2 bg-white text-zinc-900 rounded-lg hover:bg-zinc-100 transition-colors font-medium flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Pendapatan */}
          <div className="keuangan-card-pendapatan bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-zinc-400 mb-1">Pendapatan</p>
            ...
            <p className="text-xs text-zinc-500 mt-4">Dari pembayaran terkonfirmasi</p>
          </div>

          {/* Pengeluaran */}
          <div className="keuangan-card-pengeluaran bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-zinc-400 mb-1">Pengeluaran</p>
                <p className="text-2xl font-bold text-white">
          ...
          {/* Keuntungan */}
          <div className="keuangan-card-keuntungan bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-zinc-400 mb-1">Keuntungan</p>
                <p className={`text-2xl font-bold ${(summary?.keuntungan || 0) >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                  {formatCurrency(summary?.keuntungan || 0)}
                </p>
              </div>
              <div className={`p-3 ${(summary?.keuntungan || 0) >= 0 ? 'bg-green-500/10' : 'bg-red-500/10'} rounded-lg`}>
                <DollarSign className={`w-5 h-5 ${(summary?.keuntungan || 0) >= 0 ? 'text-green-500' : 'text-red-500'}`} />
              </div>
            </div>
        ...
        {/* Pengeluaran List */}
        <div className="keuangan-expense-table bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
          <div className="p-6 border-b border-zinc-800">
            <h2 className="text-lg font-semibold text-white">Rincian Pengeluaran</h2>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-zinc-900/50 border-b border-zinc-800">
                <tr>
                  <th className="text-left px-6 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wider">Tanggal</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wider">Kategori</th>
                  <th className="text-left px-6 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wider">Nama</th>
                  <th className="text-right px-6 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wider">Jumlah</th>
                  <th className="text-center px-6 py-3 text-xs font-medium text-zinc-400 uppercase tracking-wider">Aksi</th>
                </tr>
              </thead>
                  ...
                  pengeluaranList.map((item) => (
                    <tr key={item.id} className="hover:bg-zinc-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-white">
                        {item.nama}
                        {item.catatan && (
                          <p className="text-xs text-zinc-500 mt-0.5">{item.catatan}</p>
                        )}
                      </td>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white focus:ring-2 focus:ring-white/20 focus:border-transparent"
                  required
                >
                  {categories.map(cat => (
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white placeholder-zinc-500 focus:ring-2 focus:ring-white/20 focus:border-transparent"
                  required
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-2 bg-zinc-800 border border-zinc-700 rounded-lg text-white placeholder-zinc-500 focus:ring-2 focus:ring-white/20 focus:border-transparent resize-none"
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  className="flex-1 px-4 py-2 bg-white text-zinc-900 rounded-lg hover:bg-zinc-100 transition-colors font-medium flex items-center justify-center gap-2"
                >
