...
- Jangan tanya lagi, LANGSUNG solusi!`;
    ...
    const isMentalQuery = query.toLowerCase().includes('mental') || 
                          query.toLowerCase().includes('psikologis') || 
                          query.toLowerCase().includes('tekanan') || 
                          query.toLowerCase().includes('percaya diri') || 
                          query.toLowerCase().includes('juara') ||
                          query.toLowerCase().includes('asesmen');

    let finalPrompt = contextPrompt;
    if (isMentalQuery) {
      finalPrompt += `\n\n[SANGAT PENTING: Kueri ini mendeteksi topik kesiapan mental. Anda WAJIB menyertakan objek JSON "mentalAssessment" di root respons dengan format lengkap:
{
  "mentalAssessment": {
    "confidenceLevel": number,
    "pressureResponseScore": number,
    "consistencyScore": number,
    "winningMentalityScore": number,
    "overallPsychologicalScore": number,
    "findings": {
      "assessment": "string (hasil temuan)",
      "recommendation": "string (rekomendasi utama)",
      "score": number,
      "description": "string (deskripsi)"
    },
    "mentalStrengths": ["string", "string"],
    "improvementAreas": ["string", "string"],
    "recommendations": ["string", "string"]
  }
}
Pastikan data ini ada dan terisi lengkap!]`;
    }
    ...
    try {
      ...
      // Failsafe fallback for mental assessment if requested but not generated by LLM
      if (isMentalQuery && !coachingResponse.mentalAssessment) {
        console.log('[Coach Agent] ⚠️ mentalAssessment object missing in LLM response, generating failsafe assessment...');
        const winRate = finalStats?.winRate || 50;
        const confidence = Math.max(45, Math.min(95, Math.round(winRate * 0.6 + 40)));
        const pressure = Math.max(40, Math.min(90, Math.round(winRate * 0.5 + 35)));
        const consistency = Math.max(50, Math.min(92, Math.round(winRate * 0.5 + 40)));
        const winningMentality = Math.max(55, Math.min(95, Math.round(winRate * 0.4 + 50)));
        const overall = Math.round((confidence + pressure + consistency + winningMentality) / 4);

        coachingResponse.mentalAssessment = {
          confidenceLevel: confidence,
          pressureResponseScore: pressure,
          consistencyScore: consistency,
          winningMentalityScore: winningMentality,
          overallPsychologicalScore: overall,
          findings: {
            assessment: "Asesmen mental tanding menunjukkan Anda memiliki potensi mentalitas juara namun masih memerlukan peningkatan Composure di bawah tekanan.",
            recommendation: "Lakukan visualisasi keberhasilan pukulan sebelum melakukan servis di poin kritis.",
            score: overall,
            description: "Tingkat ketegangan cenderung meningkat saat menghadapi match point lawan, yang memengaruhi akurasi pukulan."
          },
          mentalStrengths: [
            "Daya juang yang konsisten di awal set",
            "Komunikasi yang baik dengan rekan partner tanding"
          ],
          improvementAreas: [
            "Kontrol emosi saat tertinggal poin berturut-turut",
            "Keberanian melakukan variasi pukulan di poin kritis"
          ],
          recommendations: [
            "Latih visualisasi pernapasan 3-detik sebelum memulai rally",
            "Simulasikan latihan game-play dengan handicap skor (misal tertinggal 18-20)"
          ]
        };
      }
    } catch (parseError) {
      ...
      if (isMentalQuery) {
        ...
        (coachingResponse as any).mentalAssessment = {
          confidenceLevel: confidence,
          pressureResponseScore: pressure,
          consistencyScore: consistency,
          winningMentalityScore: winningMentality,
    ...
    return NextResponse.json({
      ...
      mentalAssessment: coachingResponse.mentalAssessment,
    });
