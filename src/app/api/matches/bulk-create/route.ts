  ...
  try {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      try {
        ...
        // Map exact casing profile names for players
        const getExactName = (name: string) => {
          const profile = profiles.find(p => p.full_name.toLowerCase() === name.toLowerCase().trim());
          return profile ? profile.full_name : name.trim();
        };

        const t1p1 = getExactName(match.team1_player1);
        const t1p2 = getExactName(match.team1_player2);
        const t2p1 = getExactName(match.team2_player1);
        const t2p2 = getExactName(match.team2_player2);

        // Create match with scores and players extracted directly from image
