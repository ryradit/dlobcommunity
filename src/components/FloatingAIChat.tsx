...
export default function FloatingAIChat() {
  ...
  const formatMessageWithCopyButton = (content: string) => {
    ...
    return parts.map((part, index) => {
      if (IS_PHONE_RE.test(part)) {
        const waNumber = toWaNumber(part);
        const senderName = userName && userName !== 'Pengguna' ? userName : null;
        const prefilledText = senderName
          ? `Halo Admin DLOB! 👋 Saya ${senderName}, ingin bertanya lebih lanjut.`
          : 'Halo Admin DLOB! 👋 Saya ingin bertanya lebih lanjut.';
        ...
        return (
          <span key={index} className="inline-flex items-center gap-2 bg-gray-200 dark:bg-zinc-700/40 px-2.5 py-1.5 rounded-lg border border-gray-300 dark:border-white/10 my-1 transition-all duration-300">
            ...
            <a
              ...
              title={`Chat WhatsApp${senderName ? ` sebagai ${senderName}` : ''}`}
              className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-500 hover:bg-green-600 active:bg-green-700 text-white text-[11px] font-bold rounded-md transition-colors shrink-0"
            >
